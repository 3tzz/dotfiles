  ...
  config = function()
    ...
    vim.api.nvim_create_autocmd("LspAttach", {
      ...
      callback = function(ev)
        ...
        keymap.set("n", "<leader>gg", "<cmd>Telescope lsp_references<CR>", opts) -- show definition, references

        opts.desc = "Go to declaration"
        keymap.set("n", "<leader>g<F12>", vim.lsp.buf.declaration, opts) -- go to declaration
    ...
    -- Grammar/spelling!!! TODO: do it cleaner
    ...
    mason_lspconfig.setup_handlers({
      ...
      -- end,
      ["ltex"] = function()
        -- Configure LaTeX server for specific filetypes (e.g., .tex, .md)
        lspconfig["ltex"].setup({
          enabled = false, -- value change for turn_script.sh: [true]
          on_attach = function(client, bufnr)
            require("ltex_extra").setup({
              load_langs = { "pl-PL", "en-US" },
              init_check = true,
              path = "/home/freetzz/.dotfiles/txt/whitelists/",
            })
          end,
          capabilities = capabilities,
          load_langs = { "pl-PL", "en-US" },
          settings = {
            ltex = {
              ...
              language = "pl-PL", -- Change value for script swap_lang.sh : [en-US]
              additionalLanguage = "en-US", -- Change value for script swap_lang.sh : [pl-PL]
              enabled = { "latex", "tex", "bib", "markdown" }, -- Specify which filetypes are enabled
              diagnosticSeverity = "info", -- Set diagnostic severity
              checkFrequency = "save", -- Check when the file is saved
              trace = { server = "verbose" }, -- Set the server trace level
            },
            filetypes = {
              "bib",
              "markdown",
